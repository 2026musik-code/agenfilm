  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-luxury-black/90 backdrop-blur-md shadow-lg py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center relative">
        ...
        {/* Search Bar Overlay */}
        <div className={`absolute inset-0 flex items-center justify-center px-4 transition-all duration-300 ${isSearchOpen ? 'opacity-100 pointer-events-auto bg-luxury-black/95' : 'opacity-0 pointer-events-none'}`}>
             <form onSubmit={handleSearch} className="w-full max-w-2xl relative flex items-center">
                <Search className="text-luxury-gold mr-3" size={20} />
                <input
                    ref={inputRef}
                    type="text"
                    value={searchQuery}
                ...
                />
                <button type="button" onClick={() => setIsSearchOpen(false)} className="ml-4 text-gray-400 hover:text-white">
                    <X size={24} />
                </button>
             </form>
        </div>
